...
st.markdown("<div class='main-title'>LG Pit Optimizer</div>", unsafe_allow_html=True)
st.markdown("<div class='subtitle'>Optimisation de fosses minières par l'algorithme de Lerch-Grossman</div>", unsafe_allow_html=True)
            ...
            with tab1:
                ...
                if 'export_dxf' in st.session_state and st.session_state.export_dxf:
                    ...
                    if st.session_state.pit_boundary:
                        ...
                        # Créer le lien de téléchargement
                        dxf_filename = f"pit_boundary_LG_{datetime.now().strftime('%Y%m%d')}.dxf"
...
if run_optimizer:
    ...
    # Exécuter l'algorithme de Lerch-Grossman
    status_text.text("Détermination de la fosse optimale...")
    progress_bar.progress(70)
    
    st.session_state.optimal_pit, st.session_state.pit_boundary = run_lerch_grossman(
        st.session_state.block_model,
    ...
    if view_mode == "Teneurs" or (view_mode != "Fosse optimale" and view_mode != "Limites de la fosse" and not st.session_state.results_ready):
        ...
        for i, block in enumerate(sampled_blocks):
            ...
            fig.add_trace(go.Mesh3d(
                ...
                name=f"Block {i}",
                hovertext=f"X: {block['real_x']}, Y: {block['real_y']}, Z: {block['real_z']}<br>Teneur: {block['grade']:.2f}%"
            ))
        ...
        fig.update_layout(
            ...
            title="Modèle de blocs coloré par teneur"
        )
        ...
        for i, block in enumerate(sampled_blocks):
            ...
            fig.add_trace(go.Mesh3d(
                ...
                name=f"Block {i}",
                hovertext=f"X: {block['real_x']}, Y: {block['real_y']}, Z: {block['real_z']}<br>Valeur: {block['value']:.2f}$"
            ))
        ...
        for i, block in enumerate(sampled_blocks):
            ...
            color = f'rgb({50+int(z_normalized*150)}, {100+int(z_normalized*100)}, {200-int(z_normalized*100)})'
            ...
            fig.add_trace(go.Mesh3d(
                ...
                color=color,
                name=f"Block {i}",
                hovertext=f"X: {block['real_x']}, Y: {block['real_y']}, Z: {block['real_z']}<br>Teneur: {block['grade']:.2f}%<br>Valeur: {block['value']:.2f}$"
            ))
        ...
        fig.update_layout(
            ...
            margin=dict(l=0, r=0, b=0, t=0),
            height=600,
            title="Limites de la fosse optimale"
        )
...
st.markdown("<div style='text-align: center; color: #566573;'>LG Pit Optimizer v1.0.0</div>", unsafe_allow_html=True)